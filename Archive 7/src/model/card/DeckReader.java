...
public class DeckReader implements Reader {
  ...
  /**
   * Reads a deck configuration file
   * and returns a list of GameCard objects with reversed influence grids.
   *
   * @param filePath the path to the deck configuration file.
  ...
  private char[][] readAndValidateGrid(Scanner scanner, String cardName) {
    ...
    for (int row = 0; row < 5; row++) {
      if (!scanner.hasNextLine()) {
        throw new IllegalArgumentException("Incomplete grid for " +
                "card: " + cardName);
      }
      String line = scanner.nextLine();
      if (line.length() != 5) {
        throw new IllegalArgumentException("Grid row must have exactly" +
                " 5 characters for card: " + cardName);
      }
