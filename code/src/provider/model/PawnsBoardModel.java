...
public interface PawnsBoardModel<C extends ProviderCard> extends ReadonlyPawnsBoardModel {
   ...
   * @throws IllegalArgumentException if the row, col, or handIdx is out of bounds. or if the target
   *                                  coordinate has a card, or if the number of pawns is less than
   *                                  the cost of the card to be placed.
   */
  ...
  /**
   * Starts the game. The hand size is the same for both players. Allows the model to recognize the
   * game as being in progress.
   *
   ...
   * @throws IllegalArgumentException if the hand size is larger than the deck or negative, or if
